...
|---|---|
| Instance placement, the conjugation, shear/mirror handling, `scene.json` schema, mesh and terrain vertex frames, winding | [geometry and placement](reference/geometry-and-placement.md) |
| Texture export and naming, UV tiling and the V-flip, alpha roles and cutoff, PBR fields, detail/parallax/emissive, normal-map convention | [textures and materials](reference/textures-and-materials.md) |
| Terrain heightmaps and holes, MicroSplat layer scales and control-map channel packing, albedo slice tiling, the colour-grade LUT format | [terrain and the colour grade](reference/terrain-and-colour-grade.md) |
| `StaticDeferredDecal` discovery without type trees, payload byte layout, material and atlas resolution, box axes, projection clipping | [decals](reference/decals.md) |
| Reading IL2CPP MonoBehaviours from raw bytes, per-class payload layouts, exfils/doors/loot/zones, bit masks | [game data](reference/game-data.md) |
| Physics colliders and their record layout, `interact_<lv>.json`, and the GameObject-name semantic layer | [colliders, interactables and semantics](reference/colliders-interactables-and-semantics.md) |
| Skeleton and skinning, animation clip decode, the animator graph, equipment binding, the `.eftchar` container | [characters and animation](reference/characters-and-animation.md) |
| Sky cubemap faces and derived colours, particle systems and flipbook atlases, the Water4 parameter set | [sky, particles and water](reference/sky-particles-and-water.md) |
| Light extraction, controller-driven lamps, what EFT does and does not ship, the SH irradiance volume format and bake math | [lighting and the SH bake](reference/lighting-and-sh-bake.md) |
| Build stages, caching and forced invalidation, the `.eftpack` directory layout, `manifest.json` schema, every binary stride and offset, self-contained packs | [build pipeline and pack format](reference/build-pipeline-and-pack-format.md) |
