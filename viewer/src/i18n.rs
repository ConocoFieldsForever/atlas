...
pub enum K {
    ...
    BuildAllTip,
    BuildAllStopped,
    BuildAllConfirmTitle,
    BuildAllConfirmBody,
    BuildAllEstimate,
    BuildAllEstimateUnknown,
    BuildAllConfirm,
    BuildSelected,
    BuildSelectedTip,
    BuildSelectedStopped,
    ImportExisting,
    ImportExistingTip,
    ImportChooseTitle,
    ImportConfirmTitle,
...
fn pair(k: K) -> [&'static str; 2] {
    ...
    match k {
        ...
        Build => ["BUILD", "СОБРАТЬ"],
        BuildAll => ["BUILD ALL MAPS", "СОБРАТЬ ВСЕ КАРТЫ"],
        BuildAllTip => [
            "Queue every missing, damaged, unstamped or outdated map. Current completed maps are skipped.",
            "Поставить в очередь все отсутствующие, повреждённые, неотмеченные или устаревшие карты. Актуальные готовые карты пропускаются.",
        ],
        BuildAllStopped => ["BUILD ALL STOPPED", "СБОРКА ВСЕХ КАРТ ОСТАНОВЛЕНА"],
        BuildAllConfirmTitle => ["BUILD ALL MAPS?", "СОБРАТЬ ВСЕ КАРТЫ?"],
        BuildAllConfirmBody => [
            "This will process every map that currently needs a build, one at a time. It may take a while.",
            "Будут обработаны все карты, которым сейчас нужна сборка, по одной. Это может занять время.",
        ],
        BuildAllEstimate => ["Approx. local estimate", "Примерная оценка на этом компьютере"],
        BuildAllEstimateUnknown => [
            "Duration varies. Atlas will show an estimate after this computer has recorded build times.",
            "Длительность зависит от условий. Atlas покажет оценку, когда на этом компьютере появятся данные о времени сборки.",
        ],
        BuildAllConfirm => ["BUILD ALL MAPS", "СОБРАТЬ ВСЕ КАРТЫ"],
        BuildSelected => ["BUILD SELECTED", "СОБРАТЬ ВЫБРАННЫЕ"],
        BuildSelectedTip => [
            "Queue only the checked maps. They build one at a time in the order shown.",
            "Поставить в очередь только отмеченные карты. Они собираются по одной в показанном порядке.",
        ],
        BuildSelectedStopped => ["SELECTED BUILD STOPPED", "СБОРКА ВЫБРАННЫХ ОСТАНОВЛЕНА"],
        ImportExisting => ["IMPORT EXISTING MAPS", "ИМПОРТ ГОТОВЫХ КАРТ"],
        ImportExistingTip => [
            "Copy completed maps from an older Atlas folder. Existing maps are kept unchanged.",
            "Скопировать готовые карты из старой папки Atlas. Существующие карты не изменяются.",
        ],
        ImportChooseTitle => [
            "Choose an older Atlas folder or its packs folder",
            "Выберите старую папку Atlas или её папку packs",
        ],
        ImportConfirmTitle => ["IMPORT EXISTING MAPS?", "ИМПОРТИРОВАТЬ ГОТОВЫЕ КАРТЫ?"],
        ImportConfirmBody => [
            "Atlas will copy completed maps one at a time. The old folder stays unchanged, and maps already installed here are skipped.",
            "Atlas скопирует готовые карты по одной. Старая папка не изменится, а уже установленные здесь карты будут пропущены.",
        ],
        ImportStart => ["IMPORT MAPS", "ИМПОРТИРОВАТЬ КАРТЫ"],
        Importing => ["IMPORTING", "ИМПОРТ"],
        Imported => ["IMPORTED", "ИМПОРТИРОВАНО"],
        ImportSkipped => ["KEPT EXISTING", "СУЩЕСТВУЮЩАЯ СОХРАНЕНА"],
        ImportComplete => ["IMPORT COMPLETE", "ИМПОРТ ЗАВЕРШЁН"],
        ImportCancelled => ["IMPORT CANCELLED", "ИМПОРТ ОТМЕНЁН"],
        Select => ["SELECT", "ВЫБРАТЬ"],
        Queued => ["QUEUED", "В ОЧЕРЕДИ"],
        Pending => ["NOT STARTED", "НЕ ЗАПУЩЕНО"],
        ...
        CustomShotsDir => ["Screenshots folder", "Папка скриншотов"],
        CustomLogsDir => ["Game logs folder", "Папка логов игры"],
        CustomDirHint => [
            "Blank = automatic. Point these at a shared folder to run the game on another PC (network paths work).",
            "Пусто = автоматически. Укажите общую папку, чтобы игра шла на другом ПК (сетевые пути работают).",
        ],
        CustomDirMissing => ["folder not found", "папка не найдена"],
        ScreenshotLocate => [
